...
namespace Cme
{
    ...
    inline std::vector<aiTextureType> textureMapTypeToAiTextureTypes(TextureMapType type)
    {
        switch (type)
        {
    ...
    public:
        TextureMap(const Texture& texture, TextureMapType type, bool isPacked = false)
            : m_TextureObj(texture), m_eType(type), m_bPacked(isPacked) {}

        Texture& getTexture() { return m_TextureObj; }
        TextureMapType getType() const { return m_eType; }
        bool isPacked() const { return m_bPacked; }
        void setPacked(bool packed) { m_bPacked = packed; }

    private:
        Texture m_TextureObj;
        TextureMapType m_eType;
