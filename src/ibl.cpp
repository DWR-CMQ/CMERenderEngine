...
namespace qrk 
{
        ...
        : buffer_(width, height), cubemapRenderHelper_(&buffer_) 
    {
        ...
        cubemap_ = buffer_.attachTexture(BufferType::COLOR_CUBEMAP_HDR, textureParams);
    }
    ...
    {
      shader.setFloat("qrk_ggxPrefilteredEnvMapMaxLOD", static_cast<float>(cubemap_.numMips - 1.0));
    }
    ...
    GGXBrdfIntegrationCalculator::GGXBrdfIntegrationCalculator(int width, int height) : buffer_(width, height)
    {
