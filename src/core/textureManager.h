...
public:
    ...
    */
     ...
     */
    const Texture1& getTexture(const std::string& key) const;
...
private:
    ...
    std::map<std::string, std::unique_ptr<Texture1>> _textureCache; // Texture cache - stores textures within their keys in std::map
};